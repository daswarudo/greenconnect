...
$currentWeekDetails = $details->filter(function ($detail) {
    $mealDate = Carbon::parse($detail->date);
    return $mealDate->between(Carbon::now()->startOfWeek(), Carbon::now()->endOfWeek());
});

// Sort meals by meal type and day of the week
$sortedDetails = $currentWeekDetails->sortBy(function ($detail) use ($mealTypeOrder) {
    $mealTypeIndex = array_search(strtolower($detail->meal_type), $mealTypeOrder);
    return $mealTypeIndex !== false ? $mealTypeIndex : count($mealTypeOrder); // Push undefined types to the end
})->sortBy(function ($detail) {
    return Carbon::parse($detail->date)->dayOfWeek; // Then sort by day of the week
});
...
foreach ($sortedDetails as $detail) {
    $mealDay = Carbon::parse($detail->date)->format('l'); // Get full day name
    $week['days'][$mealDay][] = $detail;
}
@endphp
...
<script>
    ...
    function toggleWeek(weekName) {
        ...
        if (weekContent.style.display === 'none') {
            ...
            toggleButton.textContent = 'Show ' + weekName;  // Change button text to 'Show' // showMealDetails: <h3>${meal.planName}</h3>
        }
    }
